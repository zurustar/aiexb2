...
* **REQ-PERF-03: 同時接続数**
    * ピーク時（朝9:00-9:30）に2,000人の同時アクセスを処理できること。
    * 負荷テストにより、上記条件下でのレスポンスタイム劣化が10%以内であることを確認すること。
* **REQ-PERF-04: APIリクエストタイムアウト**
    * すべてのAPIリクエストは30秒でタイムアウトし、適切なエラーレスポンス（408 Request Timeout）を返すこと。
    * AI機能など処理時間が長い特定のエンドポイントは、個別にタイムアウト値を設定可能とすること（例: 10秒）。
    * タイムアウト発生時は、クライアント側でリクエストをキャンセル（AbortController）できること。
* **REQ-PERF-05: クロスタブセッション同期**
    * ユーザーが複数のブラウザタブでアプリケーションを開いている場合、一方のタブでのログイン/ログアウト操作は1秒以内に他のタブに反映されること。
    * セッション状態の同期には、ブラウザの `storage` イベントを使用すること。
    * 認証トークンの有効期限が5分以内の場合、自動的にトークンを更新すること。
