...
| **UC-02** | **AIアシスタントによるスマート日程調整 (Future Scope)** | AI | 自然言語による複数人の日程調整（将来実装）。 |
...
| **UC-09** | **AIによる会議準備サジェスト (Future Scope)** | AI | 議事録・テンプレートのレコメンド（将来実装）。 |
...
### UC-02: AIアシスタントによるスマート日程調整 (Future Scope)
> **Note:** 本ユースケースは将来のリリースで実装予定です。
...
### UC-09: AIによる会議準備サジェスト (Future Scope)
> **Note:** 本ユースケースは将来のリリースで実装予定です。
