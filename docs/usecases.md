...
| **UC-02** | **高度検索による日程調整** | 検索 | 複合条件による空き時間検索と候補提示。 |
| **UC-02B** | **AIアシスタントによるスマート日程調整 (Phase 2)** | AI | 自然言語による複数人の日程調整（Phase 2実装）。 |
| **UC-03** | **代理人による予定登録** | 権限 | 秘書による代理操作（既存通り）。 |
| **UC-04** | **公開範囲設定とプライバシー制御** | プライバシー | 非公開/時間枠のみ公開などの公開レベル設定。
...
| **UC-09** | **AIによる会議準備サジェスト (Phase 2)** | AI | 議事録・テンプレートのレコメンド（Phase 2実装）。 |
| **UC-10** | **予定のキャンセルとペナルティ確認** | 運用 | キャンセルポリシーに基づく警告と実行。 |
...
### UC-02: 高度検索による日程調整
複合条件による効率的な空き時間検索機能です。

* **アクター:** 一般社員
* **事前条件:** ログイン済みで、高度検索画面が開かれていること。
* **基本フロー:**
    1.  ユーザーは検索条件を入力する：
        - 参加者：「田中部長」「佐藤課長」「自分」
        - 期間：「来週」
        - 時間帯：「午後（13:00-17:00）」
        - 所要時間：「1時間」
        - リソース条件：「プロジェクター付き」「収容人数4名以上」
    2.  システムは、全参加者のスケジュールと条件に合致するリソースをクロス検索する。
    3.  システムは、候補日時を優先度順（移動時間、設備充実度等）で表示する。
    4.  ユーザーは候補から1つを選択し、予約を確定する。
* **代替フロー:**
    * **3a. 完全合致なし:** 条件に完全合致する候補がない場合、システムは代替案を提示する：
        - 時間帯を変更した候補（「午前中なら空いています」）
        - リソースを変更した候補（「プロジェクターなしなら利用可能」）
        - 参加者を調整した候補（「田中部長抜きなら可能」）

### UC-02B: AIアシスタントによるスマート日程調整 (Phase 2)
> **Note:** 本ユースケースはPhase 2での実装予定です。

1万人規模でこそ真価を発揮する、AI活用の目玉機能です。

* **アクター:** ユーザー (General/Secretary)
