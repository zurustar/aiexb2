...
### 4.1 パフォーマンス要件
*   **API応答時間:** 通常のCRUD操作は **200ms以内** (p95)、複雑な空き時間検索は **500ms以内** (p95)。
*   **API タイムアウト:** すべてのAPIリクエストは **30秒** でタイムアウトし、適切なエラーメッセージを返すこと。AI機能など時間のかかる処理は個別にタイムアウトを設定可能とする（例: 10秒）。
*   **同時接続:** ピーク時（朝9:00-9:30）に **2,000人** の同時アクセスを捌けること。
*   **セッション同期:** 複数タブでアプリケーションを開いた場合、認証状態の変更（ログイン/ログアウト）は **1秒以内** に他のタブに反映されること。
*   **トークン自動更新:** 認証トークンの有効期限切れを防ぐため、期限の **5分前** に自動的にトークンを更新すること。
