...
graph TD
    ...
    BE2 -->|SAML/OIDC| IdP
    BE1 -->|Logs| Fluent[Fluent Bit]
    BE2 -->|Logs| Fluent
    Fluent -->|Forward| LogAggregator[Log Aggregator/S3]
    BE1 -->|Metrics| Prom[Prometheus/Datadog]
    BE2 -->|Metrics| Prom
```
...
*   **キャッシュ戦略:** リソース空き状況は競合回避のためキャッシュなし。個人カレンダー等は短期キャッシュ（TTL 5分）で性能向上。権限データは15分、リソースマスタは30分。予約操作時は操作者本人のキャッシュを即座無効化、他の参加者は非同期無効化。
*   **バックアップ/DR:** RPO 15分、RTO 1時間。DBは自動スナップショット+PITR。クロスリージョンレプリケーションまたはIaCによるDR環境構築。四半期ごとの復旧演習を実施。
...
*   **可観測性:** OpenTelemetry によるトレース/メトリクス/ログ収集。構造化JSONログ出力。主要メトリクスはREDメソッド（Rate, Errors, Duration）に基づき収集。SLO違反時はPagerDuty/Slackへ通知。
*   **コスト管理:** 環境（dev/stg/prod）ごとにコストを分離し、予算超過アラートを設定。

#### Phase 2 追加ジョブ・可観測性
*   **追加ジョブ種別:** AIバッチ（要約/レコメンドの事前生成）、ベクトルDB同期、学習データ収集。
*   **追加メトリクス:** LLM呼び出し成功率・平均コスト、ベクトル検索レイテンシ、AI機能利用率。
*   **AIガバナンス:** AIサジェスト採用率・誤提案率・NPSを計測し、品質低下時はキルスイッチ発動。学習データへのPII混入防止、再学習禁止データポリシー遵守。モデル更新はオフライン評価+A/Bテスト必須。
...
### 6.1 セキュリティ実装
*   **認証・認可:** SSO (SAML 2.0/OIDC) 必須、MFAはIdP側で強制。RBACはポリシーバージョニング管理。
*   **秘密情報管理:** AWS Secrets Manager / KMS を使用し、コードへのハードコード禁止。
*   **脆弱性管理:** CIパイプラインでSCA/コンテナスキャンを実施、Critical/High脆弱性をブロック。
*   **監査ログ:** 操作ログを1年間オンライン保持、その後コールドストレージへ移行（計3年）。改ざん検知用ハッシュ付与。

### 6.2 データライフサイクル
*   **保持期間:** 業務データ3年、監査ログ3年。
*   **PII保護:** 保存時に最小化・マスキング。分析用データは匿名化・仮名化処理を実施。

### 6.3 開発・運用プロセス
*   **CI/CD:** Lint、テスト、セキュリティスキャンを通過したコードのみマージ。DBマイグレーションは後方互換性維持。
*   **品質保証:** 単体・統合テストカバレッジ80%以上。OpenAPI契約テスト必須。
*   **障害対応:** Redis障害時はDBフォールバック、SSO障害時はRead-Onlyモード。
