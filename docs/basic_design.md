...
*   **ジョブ種別:** 通知送信（メール/チャット、リトライ3回・指数バックオフ）、未チェックイン予約の自動解放、承認リマインダ、エクスポート/バックアップ転送。冪等性キーは予約ID＋ジョブ種別で管理し、3回失敗でデッドレターへ送る。
...
| **FN-12** | 高度検索 | 複数条件での空き時間検索・フィルタリング機能（参加者・期間・リソース条件の組み合わせ）。 | - |

#### Phase 2 拡張機能（将来実装）
| 機能ID | 機能名称 | 概要 | 対応SRS |
| :--- | :--- | :--- | :--- |
| **FN-04** | AI日程調整 | 自然言語による複数人の日程調整。 | REQ-06 |
| **FN-09** | 会議準備支援 | AI による議事録・テンプレートのレコメンド。 | REQ-10 |
