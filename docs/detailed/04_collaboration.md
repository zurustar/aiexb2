...
### 5.3 通知戦略 (Notification Strategy)
...
### 5.4 組織ポリシー変更時の運用フック
-   **設定の外部化:** 適用条件（人数閾値、対象リソース、役職リスト）は `approval_policy` テーブルに保持し、管理画面から変更可能とする。
-   **デプロイレス反映:** ポリシー変更時は設定テーブルに対する更新イベントを発火し、バックエンドのポリシーキャッシュを即時無効化するWebhookを用意する。
-   **監査ログ:** 変更前後の値と変更者を監査ログに記録し、障害時は直近のバージョンへロールバック可能にする（`policy_version` を付与）。
