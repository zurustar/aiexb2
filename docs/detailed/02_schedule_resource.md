...
#### 8.2.1 データ種別別キャッシュ戦略
ユースケース分析に基づき、競合リスクとパフォーマンスを考慮したキャッシュ戦略を採用。

| データ種別 | キャッシュ戦略 | TTL | 理由 |
| :--- | :--- | :--- | :--- |
| **リソース空き状況** | **キャッシュなし** | - | 予約競合の原因となるため、常に最新データを参照 |
| **個人カレンダー** | 短期キャッシュ | 5分 | 個人データで競合なし、リロードで解決可能 |
| **予定詳細** | 短期キャッシュ | 5分 | 参照のみ、影響範囲限定 |
| **ユーザー権限情報** | 中期キャッシュ | 15分 | 変更頻度低、セキュリティ影響考慮 |
| **リソースマスタ** | 長期キャッシュ | 30分 | 静的データ、変更頻度極低 |

#### 8.2.2 キャッシュキー設計
統一的なネーミング規則による管理性向上。

**キーパターン:**
- 個人カレンダー: `calendar:user:{user_id}:date:{YYYY-MM-DD}`
- 予定詳細: `event:detail:{event_id}`
- ユーザー権限: `user:permissions:{user_id}`
- リソースマスタ: `resource:master:{resource_id}`

#### 8.2.3 無効化戦略
予約操作時の段階的キャッシュ無効化。操作者本人には即座反映、他の参加者には遅延反映。
...
**ユーザー体験の保証:**
- 操作者：変更が即座にカレンダーに反映される
- 他の参加者：最大5分の遅延で反映される（許容範囲）
...
#### 9.1.1 自動パーティション管理
現在年から2年先までのパーティションを自動的に作成・維持する仕組み。
...
**運用フロー:**
1. 現在年+2年分のパーティション必要性を判定
2. 存在しないパーティションを特定
3. 必要なパーティションを順次作成
4. 作成結果をログに記録

#### 9.1.2 パーティション戦略変更対応
運用開始後のパーティション戦略変更に対応する設計。

**設定管理:**
- パーティション種別（yearly/monthly/quarterly）
- データ保持期間の設定
- 自動作成の有効/無効制御
...
#### 9.2.1 性能監視
データベースの性能状況を継続的に監視し、最適化の必要性を判定。
...
**分析項目:**
- クエリ実行時間の分布
- インデックススキャン vs フルテーブルスキャン
- キャッシュヒット率
- 未使用インデックスの特定

#### 9.2.2 インデックス最適化提案
実際の使用パターンに基づく自動的なインデックス提案システム。

**提案ロジック:**
- 遅いクエリパターンの解析
- 頻繁なWHERE句・ORDER BY句の特定
- 複合インデックスの候補生成
- インデックス種別（B-tree/GiST/GIN）の推奨

**優先度判定:**
- 影響度（クエリ頻度×改善効果）
- 実装コスト（インデックスサイズ、作成時間）
- 運用リスク（ロック時間、ストレージ使用量）

### 9.3 運用監視・アラート

#### 9.3.1 性能メトリクス収集
システムの健全性を示す主要指標の継続的な収集。

**収集メトリクス:**
- データベース接続数（アクティブ/アイドル）
- クエリ実行時間の分布
- 遅いクエリの発生頻度
- インデックス効率性の指標
...
##### Master-Slave構成
- **Master**: 書き込み専用、キャッシュ更新・無効化処理
- **Slave**: 読み取り専用、負荷分散とフェイルオーバー対応
- **レプリケーション**: 非同期レプリケーション、遅延監視

##### メモリ管理
- **使用量想定**: 2,000人同時接続時で約8GB（個人カレンダー中心）
- **退避ポリシー**: LRU（Least Recently Used）による自動退避
- **メモリ監視**: 使用率80%でアラート、90%で緊急対応

##### 可用性・障害対応
- **ヘルスチェック**: 1秒間隔でのpingチェック
- **フェイルオーバー**: Master障害時の自動昇格（30秒以内）
- **フォールバック**: Redis全断時はDB直接アクセスに自動切替

##### キャッシュ効果測定
- **ヒット率目標**: 個人カレンダー 85%以上、予定詳細 70%以上
- **レスポンス改善**: キャッシュヒット時 50ms以下、ミス時 200ms以下
- **負荷軽減**: DB読み取り負荷 30%削減を目標
##
## 8.2.5 無効化パターン詳細
...
即座無効化:
- calendar:user:tanaka:date:2025-12-01 (田中さんのカレンダー)
- event:detail:{event_id} (予定詳細)

非同期無効化:
- calendar:user:sato:date:2025-12-01 (参加者佐藤さんのカレンダー)
- calendar:user:yamada:date:2025-12-01 (参加者山田さんのカレンダー)
```

##### パターン2: 会議の参加者追加
```
操作者: 田中さんが会議に新しい参加者を追加
...
非同期無効化:
- calendar:user:新参加者:date:2025-12-01 (新参加者のカレンダー)
- calendar:user:既存参加者:date:2025-12-01 (既存参加者のカレンダー)
```

##### パターン3: 予定のキャンセル
```
操作者: 田中さんが主催する会議をキャンセル
...
##### 無効化タイミング保証
- **同期無効化**: API応答前に完了（レスポンス時間に含む）
- **非同期無効化**: 5秒以内に開始、30秒以内に完了
- **失敗時対応**: 無効化失敗時はTTL期限切れで自然解決（最大5分）